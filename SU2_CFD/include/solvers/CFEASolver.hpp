...
public:
   ...
   * \param[in] config - Definition of the problem.
   */
  inline su2double Evaluate_ConstrFunc(const CConfig *config, unsigned short iConstr) final {
    su2double ConstraintFunction = 0.0;
    switch (config->GetKind_ConstrFunc(iConstr)) {
      case REFERENCE_GEOMETRY:
        ConstraintFunction = GetTotal_OFRefGeom();
        break;
      case REFERENCE_NODE:
        ConstraintFunction = GetTotal_OFRefNode();
        break;
      case TOPOL_COMPLIANCE:
        ConstraintFunction = GetTotal_OFCompliance();
        break;
      case VOLUME_FRACTION:
        ConstraintFunction = GetTotal_OFVolFrac();
        break;
      case TOPOL_DISCRETENESS:
        ConstraintFunction = GetTotal_OFDiscreteness();
        break;
      case STRESS_PENALTY:
        ConstraintFunction = GetTotal_OFStressPenalty();
        break;
    }
