...
void CFVMFlowSolverBase<V, R>::Allocate(const CConfig& config) {
  ...
  ConstraintFunction.resize(config.GetnConstr(),0.0);
...
template<class V, ENUM_REGIME R>
su2double CFVMFlowSolverBase<V,R>::EvaluateCommonConstrFunc(const CConfig& config, unsigned short iConstr) const {
  ...
  for (auto iMarker = 0u; iMarker < config.GetnMarker_Monitoring(); iMarker++) {
    ...
    switch (config.GetKind_ConstrFunc(iConstr)) {
      case DRAG_COEFFICIENT:
        objFun += SurfaceCoeff.CD[iMarker];
        break;
      case LIFT_COEFFICIENT:
        objFun += SurfaceCoeff.CL[iMarker];
        break;
      case SIDEFORCE_COEFFICIENT:
        objFun += SurfaceCoeff.CSF[iMarker];
        break;
      case MOMENT_X_COEFFICIENT:
        objFun += SurfaceCoeff.CMx[iMarker];
        break;
      case MOMENT_Y_COEFFICIENT:
        objFun += SurfaceCoeff.CMy[iMarker];
        break;
      case MOMENT_Z_COEFFICIENT:
        objFun += SurfaceCoeff.CMz[iMarker];
        break;
      case FORCE_X_COEFFICIENT:
        objFun += SurfaceCoeff.CFx[iMarker];
        break;
      case FORCE_Y_COEFFICIENT:
        objFun += SurfaceCoeff.CFy[iMarker];
        break;
      case FORCE_Z_COEFFICIENT:
        objFun += SurfaceCoeff.CFz[iMarker];
        break;
      case TOTAL_HEATFLUX:
        objFun += Surface_HF_Visc[iMarker];
        break;
      case MAXIMUM_HEATFLUX:
        objFun += Surface_MaxHF_Visc[iMarker];
        break;
      default:
        break;
    }
  ...
  switch (config.GetKind_ConstrFunc(iConstr)) {
    ...
    case SURFACE_MASSFLOW:
      ...
      break;
    case SURFACE_UNIFORMITY:
      objFun += config.GetSurface_Uniformity(0);
      break;
    case SURFACE_SECONDARY:
      objFun += config.GetSurface_SecondaryStrength(0);
      break;
    case SURFACE_MOM_DISTORTION:
      objFun += config.GetSurface_MomentumDistortion(0);
      break;
    case SURFACE_SECOND_OVER_UNIFORM:
      objFun += config.GetSurface_SecondOverUniform(0);
      break;
    case SURFACE_PRESSURE_DROP:
      objFun += config.GetSurface_PressureDrop(0);
      break;
    case CUSTOM_OBJFUNC:
