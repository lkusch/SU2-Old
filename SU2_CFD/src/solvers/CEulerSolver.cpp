...
su2double CEulerSolver::Evaluate_ConstrFunc(const CConfig *config, unsigned short iConstr) {

  unsigned short iMarker_Monitoring, Kind_ConstrFunc;

  ConstraintFunction[iConstr] = EvaluateCommonConstrFunc(*config, iConstr);

  /*--- Loop over all monitored markers, add to the 'combo' constraint ---*/
  ...
  for (iMarker_Monitoring = 0; iMarker_Monitoring < config->GetnMarker_Monitoring(); iMarker_Monitoring++) {
    ...
    switch(Kind_ConstrFunc) {
      case DRAG_COEFFICIENT:
        if (config->GetFixed_CL_Mode()) ConstraintFunction[iConstr] -= config->GetdCD_dCL()*(SurfaceCoeff.CL[iMarker_Monitoring]);
        if (config->GetFixed_CM_Mode()) ConstraintFunction[iConstr] -= config->GetdCD_dCMy()*(SurfaceCoeff.CMy[iMarker_Monitoring]);
        break;
      case MOMENT_X_COEFFICIENT:
        if (config->GetFixed_CL_Mode()) ConstraintFunction[iConstr] -= config->GetdCMx_dCL()*(SurfaceCoeff.CL[iMarker_Monitoring]);
        break;
      case MOMENT_Y_COEFFICIENT:
        if (config->GetFixed_CL_Mode()) ConstraintFunction[iConstr] -= config->GetdCMy_dCL()*(SurfaceCoeff.CL[iMarker_Monitoring]);
        break;
      case MOMENT_Z_COEFFICIENT:
        if (config->GetFixed_CL_Mode()) ConstraintFunction[iConstr] -= config->GetdCMz_dCL()*(SurfaceCoeff.CL[iMarker_Monitoring]);
        break;
      default:
        break;
    }
  ...
  /*--- The following are not per-surface, and so to avoid that they are
   double-counted when multiple surfaces are specified, they have been
   placed outside of the loop above. ---*/
  ...
  switch(Kind_ConstrFunc) {
    ...
    case NEARFIELD_PRESSURE:
      ...
      break;
    case SURFACE_MACH:
      ConstraintFunction[iConstr]+=config->GetSurface_Mach(0);
      break;
    default:
      break;
  }
