...
void DiscAdjMeanFlowIterationProjected(COutput *output, CIntegration ***integration_container, CGeometry ***geometry_container,
                          CSolver ****solver_container, CNumerics *****numerics_container, CConfig **config_container,
                          CSurfaceMovement **surface_movement, CVolumetricMovement **volume_grid_movement, CFreeFormDefBox*** FFDBox) {
  ...
  //UpdateMesh
  system("cp mesh_NACA0012_inv_original.su2 mesh_NACA0012_inv.su2");
  if(ExtIter>100){ //TODO >0


     system("python ../../su2-oneshot-old/SU2_PY/projectanddeform.py -f continuousAdj.cfg -n 1");
     system("cp mesh_NACA0012_inv_deform.su2 mesh_NACA0012_inv.su2");
     system("rm config*");
  ...
  if (ExtIter == 0 || config_container[ZONE_0]->GetOne_Shot()){
    ...
    for (iZone = 0; iZone < nZone; iZone++){
      ...
      for (iMesh = 0; iMesh <= config_container[iZone]->GetnMGLevels(); iMesh++){
        ...
        if (flow){
          ...
          //solver_container[iZone][iMesh][ADJFLOW_SOL]->OutputWritten(geometry_container[iZone][iMesh]);
        }
      }
    }
    ...
    for (iZone = 0; iZone < nZone; iZone++){
        for (iMesh = 0; iMesh <= config_container[iZone]->GetnMGLevels(); iMesh++){
            solver_container[iZone][iMesh][ADJFLOW_SOL]->StoreOldSolution();
        }
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    ...
    /*for (iMesh = 0; iMesh <= config_container[iZone]->GetnMGLevels(); iMesh++){
      if (flow){
        solver_container[iZone][iMesh][ADJFLOW_SOL]->SetAdjointOutput(geometry_container[iZone][iMesh], //TODO: Update
                                                                      config_container[iZone]);
      }
    }*/
    solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetAdjointOutputUpdate(geometry_container[iZone][MESH_0],
                                                                  config_container[iZone]);
  }
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    if (flow){
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetSensitivity(geometry_container[iZone][MESH_0],config_container[iZone]); //contains alpha*Deltay^T G_u (Attention: overwrites Nu!)
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->UpdateLagrangeSensitivity(geometry_container[iZone][MESH_0],200.0);
    }
    for (iMesh = 0; iMesh <= config_container[iZone]->GetnMGLevels(); iMesh++){
      if (flow){
        solver_container[iZone][iMesh][ADJFLOW_SOL]->SetAdjointInputHelp(geometry_container[iZone][iMesh],
                                                                     config_container[iZone]);
      }
    }
  }
  ...
  for (iZone = 0; iZone < nZone; iZone++){
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->UpdateStateVariable();
  }
...
if (config_container[ZONE_0]->GetOne_Shot()) AD::globalTape.reset();
  ...
  /*for (iZone = 0; iZone < nZone; iZone++){
    ...
    for (iMesh = 0; iMesh <= config_container[iZone]->GetnMGLevels(); iMesh++){
        solver_container[iZone][iMesh][ADJFLOW_SOL]->OutputWritten();
    }
  }*/


  //Restore old solutions
  //load old solution
  for (iZone = 0; iZone < nZone; iZone++){
  ...
  if (ExtIter == 0 || config_container[ZONE_0]->GetOne_Shot()){
    ...
    //store old solution //TODO: ist das richtig?
    /*for (iZone = 0; iZone < nZone; iZone++){
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->StoreOldSolution();
    }*/
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    ...
    if (flow){
      ...
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->UpdateLagrangeSensitivity(geometry_container[iZone][MESH_0],2.0);
    }
  }

  for (iZone = 0; iZone < nZone; iZone++){

    solver_container[iZone][MESH_0][ADJFLOW_SOL]->BFGSUpdate(geometry_container[iZone][MESH_0],config_container[iZone],ExtIter);
    solver_container[iZone][MESH_0][ADJFLOW_SOL]->DesignUpdate(geometry_container[iZone][MESH_0],config_container[iZone]);
    solver_container[iZone][MESH_0][ADJFLOW_SOL]->OverwriteSensitivity(geometry_container[iZone][MESH_0]);
    output->SetSurfaceCSV_Adjoint(config_container[iZone], geometry_container[iZone][MESH_0], solver_container[iZone][MESH_0][ADJFLOW_SOL], solver_container[iZone][MESH_0][FLOW_SOL], ExtIter, iZone);
  }
...
void DiscAdjMeanFlowIterationOld(COutput *output, CIntegration ***integration_container, CGeometry ***geometry_container,
                          CSolver ****solver_container, CNumerics *****numerics_container, CConfig **config_container,
                          CSurfaceMovement **surface_movement, CVolumetricMovement **volume_grid_movement, CFreeFormDefBox*** FFDBox) {

  unsigned short iZone, iMesh, ExtIter = config_container[ZONE_0]->GetExtIter();
  //std::cout<<ExtIter<<std::endl;
  unsigned short nZone = config_container[ZONE_0]->GetnZone();
...
void DiscAdjMeanFlowIteration(COutput *output, CIntegration ***integration_container, CGeometry ***geometry_container,
  ...
  //UpdateMesh
  //system("cp mesh_NACA0012_inv_original.su2 mesh_NACA0012_inv.su2");
  if(ExtIter==config_container[0]->GetOneShotStart()+1){
      deformOneShot(output, geometry_container, config_container, surface_movement, volume_grid_movement, solver_container, false);
  }
  if(ExtIter>config_container[0]->GetOneShotStart()+1){ //&& ExtIter<270){
    deformOneShot(output, geometry_container, config_container, surface_movement, volume_grid_movement, solver_container, true);
  }
  /*if(ExtIter==0){
    deformOneShot(output, geometry_container, config_container, surface_movement, volume_grid_movement, solver_container, false);
  }*/
  /*if(ExtIter>100){ //TODO >0
     std::cout<<"CHANGEMESH"<<std::endl;
     solver_container[0][MESH_0][ADJFLOW_SOL]->WriteDesignVariable();
     system("python ../../su2-oneshot-old/SU2_PY/deform.py -f continuousAdj.cfg -n 1");
     system("cp mesh_NACA0012_inv_deform.su2 mesh_NACA0012_inv.su2");
     system("rm config*");
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    if (flow){
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetSensitivity(geometry_container[iZone][MESH_0],config_container[iZone]); //contains alpha*Deltay^T G_u (Attention: overwrites Nu!)
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->UpdateLagrangeSensitivity(geometry_container[iZone][MESH_0],config_container[ZONE_0]->GetOneShotAlpha()); //vorher 200
    }
    for (iMesh = 0; iMesh <= config_container[iZone]->GetnMGLevels(); iMesh++){
      if (flow){
        solver_container[iZone][iMesh][ADJFLOW_SOL]->SetAdjointInputHelp(geometry_container[iZone][iMesh],
...
if (config_container[ZONE_0]->GetOne_Shot()) AD::globalTape.reset();
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    solver_container[iZone][MESH_0][ADJFLOW_SOL]->OverwriteSensitivityProjected(geometry_container[iZone][MESH_0]); //ACHTUNG: AENDERUNG!!!
    //output->SetSurfaceCSV_Adjoint(config_container[iZone], geometry_container[iZone][MESH_0], solver_container[iZone][MESH_0][ADJFLOW_SOL], solver_container[iZone][MESH_0][FLOW_SOL], ExtIter, iZone);
    //system("python ../../su2-oneshot-old/SU2_PY/project.py -f continuousAdj.cfg -n 1");
    ...
    solver_container[iZone][MESH_0][ADJFLOW_SOL]->DesignUpdateProjected(geometry_container[iZone][MESH_0],config_container[iZone], ExtIter);
...
void DiscAdjMeanFlowIterationConstraint(COutput *output, CIntegration ***integration_container, CGeometry ***geometry_container,
  ...
  if(ExtIter>config_container[ZONE_0]->GetOneShotStart()){ //&& ExtIter<6000){
    solver_container[iZone][MESH_0][ADJFLOW_SOL]->UpdateMultiplier(config_container[0]);
    deformOneShot(output, geometry_container, config_container, surface_movement, volume_grid_movement, solver_container, true);
  }
  ...
  if (ExtIter == 0 || config_container[ZONE_0]->GetOne_Shot()){
    ...
    for (iZone = 0; iZone < nZone; iZone++){
      ...
      if (flow){
        solver_container[iZone][MESH_0][ADJFLOW_SOL]->RegisterObj_Func(config_container[iZone]);
        solver_container[iZone][MESH_0][ADJFLOW_SOL]->RegisterConstraint_Func(config_container[iZone]);
      }
      for (iMesh = 0; iMesh <= config_container[iZone]->GetnMGLevels(); iMesh++){
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    ...
    for (iMesh = 0; iMesh <= config_container[iZone]->GetnMGLevels(); iMesh++){
      if (flow){
        solver_container[iZone][iMesh][ADJFLOW_SOL]->SetAdjointOutputZero(geometry_container[iZone][iMesh],
                                                                      config_container[iZone]);
      }
    }
  }
  ...
  if(SU2_TYPE::GetPrimary(solver_container[0][MESH_0][ADJFLOW_SOL]->GetConstraintFunc_Value())<0){
      if(config_container[0]->GetEqualConstraint()){
          solver_container[0][MESH_0][ADJFLOW_SOL]->SetMultiplier(config_container[0], -SU2_TYPE::GetPrimary(normfu/normgu)); //TODO 0.0
      }else{
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    ...
    if (flow){
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetAdj_ObjFunc(geometry_container[iZone][MESH_0], config_container[iZone],1.0);
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetAdj_ConstraintFunc(geometry_container[iZone][MESH_0], config_container[iZone],solver_container[iZone][MESH_0][ADJFLOW_SOL]->GetMultiplier());
    }
    for (iMesh = 0; iMesh <= config_container[iZone]->GetnMGLevels(); iMesh++){
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    solver_container[iZone][MESH_0][ADJFLOW_SOL]->StoreConstraint();
  }
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    if (flow){
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetAdj_ObjFunc(geometry_container[iZone][MESH_0], config_container[iZone],0.0);
    //  if(SU2_TYPE::GetPrimary(solver_container[0][MESH_0][ADJFLOW_SOL]->GetConstraintFunc_Value())>0){
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetAdj_ConstraintFunc(geometry_container[iZone][MESH_0], config_container[iZone],SU2_TYPE::GetPrimary(solver_container[iZone][MESH_0][ADJFLOW_SOL]->GetConstraintFunc_Value()));
    /*}else{
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetAdj_ConstraintFunc(geometry_container[iZone][MESH_0], config_container[iZone],0.0);
      }*/
    }
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    if (flow){
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetSensitivity(geometry_container[iZone][MESH_0],config_container[iZone]); //contains alpha*Deltay^T G_u
...
if (config_container[ZONE_0]->GetOne_Shot()) AD::globalTape.reset();
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    ...
    if (flow){
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->SetSensitivityFD(geometry_container[iZone][MESH_0],config_container[iZone]); //contains N_u_new
      solver_container[iZone][MESH_0][ADJFLOW_SOL]->UpdateLagrangeSensitivity(geometry_container[iZone][MESH_0],20.0); //2
    }
  }
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    ...
    solver_container[iZone][MESH_0][ADJFLOW_SOL]->BFGSUpdateProjected(geometry_container[iZone][MESH_0],config_container[iZone],ExtIter);
    //solver_container[iZone][MESH_0][ADJFLOW_SOL]->UpdateMultiplier();
    solver_container[iZone][MESH_0][ADJFLOW_SOL]->DesignUpdateProjected(geometry_container[iZone][MESH_0],config_container[iZone], ExtIter);
  }
...
void projectOneShot(CGeometry ***geometry_container, CConfig **config_container,
    ...
    unsigned short iMarker, iDim, iDV;
    ...
    //geometry_container[ZONE_0][MESH_0]->SetBoundSensitivity(config_container[ZONE_0]);
    ...
    /*--- Boolean controlling points to be updated ---*/
      ...
      ofstream myfile;
      myfile.open ("projectedLagrangian.txt");*/
  ...
  #endif
        //myfile <<std::setprecision(16)<<Gradient<<std::endl;
...
void deformOneShot(COutput *output, CGeometry ***geometry_container, CConfig **config_container,
                       CSurfaceMovement **surface_movement, CVolumetricMovement **grid_movement, CSolver ****solver_container, bool deformit) {
    ...
    /*if(deformit){
    for (iDV = 0; iDV < config_container[ZONE_0]->GetnDV(); iDV++) {
          //config_container[ZONE_0]->SetDV_Value(iDV, (-1)*solver_container[ZONE_0][MESH_0][ADJFLOW_SOL]->getDesignVar(iDV));
          config_container[ZONE_0]->SetDV_Value(iDV, -0.0001);
    }
    }
    ...
    for (iDV = 0; iDV < config_container[ZONE_0]->GetnDV(); iDV++) {
      //if (deformit){
          ...
          //config_container[ZONE_0]->SetDV_Value(iDV, solver_container[ZONE_0][MESH_0][ADJFLOW_SOL]->getDesignVar(iDV)+solver_container[ZONE_0][MESH_0][ADJFLOW_SOL]->getDVValue(iDV));
          //config_container[ZONE_0]->SetDV_Value(iDV, 0.0001+solver_container[ZONE_0][MESH_0][ADJFLOW_SOL]->getDVValue(iDV));
          std::cout<<solver_container[ZONE_0][MESH_0][ADJFLOW_SOL]->getDVValue(iDV)<<" ";
          if(solver_container[ZONE_0][MESH_0][ADJFLOW_SOL]->getDVValue(iDV)!=0)   updatecount=updatecount+1;   //updatebool=true;
      /*}else{
          config_container[ZONE_0]->SetDV_Value(iDV, 0.0005);
      }*/
    ...
    if (rank == MASTER_NODE) cout << "Finished Deformation." << endl;
    }
    ...
    //surface_movement[ZONE_0]->WriteFFDInfo(geometry_container[ZONE_0][MESH_0], config_container[ZONE_0]);
