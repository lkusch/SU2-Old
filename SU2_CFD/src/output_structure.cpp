...
void COutput::MergeSolution(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  switch (Kind_Solver) {
    ...
    case DISC_ADJ_EULER: case DISC_ADJ_NAVIER_STOKES: case ONE_SHOT_EULER: case ONE_SHOT_NAVIER_STOKES: FirstIndex = ADJFLOW_SOL; SecondIndex = NONE; ThirdIndex = NONE; break;
    case DISC_ADJ_RANS: case ONE_SHOT_RANS: FirstIndex = ADJFLOW_SOL; if (config->GetFrozen_Visc_Disc()) SecondIndex = NONE; else SecondIndex = ADJTURB_SOL; ThirdIndex = NONE; break;
...
void COutput::SetConvHistory_Header(ofstream *ConvHist_file, CConfig *config, unsigned short val_iZone) {
  ...
  bool turbulent = ((config->GetKind_Solver() == RANS) || (config->GetKind_Solver() == ADJ_RANS) ||
                    (config->GetKind_Solver() == DISC_ADJ_RANS) || (config->GetKind_Solver() == ONE_SHOT_RANS));
...
void COutput::SetConvHistory_Body(ofstream *ConvHist_file,
  ...
  if (rank == MASTER_NODE) {
    ...
    bool turbulent = ((config[val_iZone]->GetKind_Solver() == RANS) || (config[val_iZone]->GetKind_Solver() == ADJ_RANS) ||
                      (config[val_iZone]->GetKind_Solver() == DISC_ADJ_RANS) || (config[val_iZone]->GetKind_Solver() == ONE_SHOT_RANS));
    ...
    switch (config[val_iZone]->GetKind_Solver()) {
      ...
      case ONE_SHOT_EULER:          case ONE_SHOT_NAVIER_STOKES:          case ONE_SHOT_RANS:
        ...
        if (output_surface) {
          ...
          Surface_TotalFlowUniformityIndex = config[ZONE_0]->GetSurface_FlowUniformityIndex(1);
    ...
    if (  (!fem && ((In_NoDualTime || In_DualTime_0 || In_DualTime_1) && (In_NoDualTime || In_DualTime_2 || In_DualTime_3))) ||
      ...
      if (!DualTime_Iteration) {
        ...
        switch (config[val_iZone]->GetKind_Solver()) {
          ...
          case EULER : case NAVIER_STOKES: case RANS:
          case ADJ_EULER: case ADJ_NAVIER_STOKES: case ADJ_RANS: case DISC_ADJ_EULER: case ONE_SHOT_EULER:
          case DISC_ADJ_NAVIER_STOKES: case DISC_ADJ_RANS: case ONE_SHOT_RANS:  case ONE_SHOT_NAVIER_STOKES:
            
            /*--- Direct coefficients ---*/
            SPRINTF (direct_coeff, ", %14.8e, %14.8e, %14.8e, %14.8e, %14.8e, %14.8e, %14.8e, %14.8e, %14.8e, %14.8e, %14.8e, %14.8e, %.20lf",
                     Total_CL, Total_CD, Total_CSF, Total_CMx, Total_CMy, Total_CMz, Total_CFx, Total_CFy,
                     Total_CFz, Total_CEff, Total_AoA, Total_Custom_ObjFunc, Surface_TotalFlowUniformityIndex);
        ...
        if (  (!fem && ((write_heads) && !(!DualTime_Iteration && Unsteady))) ||
          ...
          case RANS :
           ...
           break;
            ...
            case ONE_SHOT_EULER:          case ONE_SHOT_NAVIER_STOKES:
            ...
            case ADJ_RANS : case DISC_ADJ_RANS: case ONE_SHOT_RANS:
...
void COutput::WriteTurboPerfConvHistory(CConfig *config){
  ...
  bool turbulent = ((config->GetKind_Solver() == RANS) || (config->GetKind_Solver() == DISC_ADJ_RANS) || (config->GetKind_Solver() == ONE_SHOT_RANS));
...
void COutput::LoadLocalData_AdjFlow(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  switch (config->GetKind_Solver()) {
    ...
    case ONE_SHOT_EULER: case ONE_SHOT_NAVIER_STOKES: FirstIndex = ADJFLOW_SOL; SecondIndex = NONE;  break;
    case ONE_SHOT_RANS: FirstIndex = ADJFLOW_SOL; if (config->GetFrozen_Visc_Disc()) SecondIndex = NONE; else SecondIndex = ADJTURB_SOL;  break;
  }
...
void COutput::SpecialOutput_AnalyzeSurface(CSolver *solver, CGeometry *geometry, CConfig *config, bool output) {
  ...
  AxiFactor = 1.0, SoundSpeed, Vn, Vn2, Vtang2, VnDiff, Weight = 1.0;
  ...
  su2double  *Surface_FlowUniformityIndex = new su2double[nMarker];
  su2double  *Vn_Mean = new su2double[nMarker];  
  ...
  for (iMarker = 0; iMarker < nMarker; iMarker++) {
    ...
    Surface_FlowUniformityIndex[iMarker] = 0.0;
    Vn_Mean[iMarker] = 0.0;
    ...
    Vn_Mean[iMarker] = Vn_Mean[iMarker]/fabs(Surface_Area[iMarker]);
   for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) { 
        iPoint = geometry->vertex[iMarker][iVertex]->GetNode();
        ...
        if (geometry->node[iPoint]->GetDomain()) {
          ...
          Velocity2 = 0.0; Area = 0.0; MassFlow = 0.0, Vn = 0.0, VnDiff = 0.0;
          for (iDim = 0; iDim < nDim; iDim++) {
            Area += (Vector[iDim] * AxiFactor) * (Vector[iDim] * AxiFactor);
            Velocity[iDim] = solver->node[iPoint]->GetVelocity(iDim);
          ...
          Area              = sqrt (Area);
          Vn                = Vn / Area;
          VnDiff            = fabs(Vn-Vn_Mean[iMarker]);
	  
	  Surface_FlowUniformityIndex[iMarker] += VnDiff*fabs(Area);
        }
      }
   
     Surface_FlowUniformityIndex[iMarker] = 1-Surface_FlowUniformityIndex[iMarker]/(2*fabs(Vn_Mean[iMarker])*fabs(Surface_Area[iMarker]));
  }
  }
  ...
  su2double * Surface_FlowUniformityIndex_Local = new su2double[nMarker_Analyze];
  ...
  su2double *Surface_FlowUniformityIndex_Total = new su2double [nMarker_Analyze];
  ...
  for (iMarker_Analyze = 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
    ...
    Surface_FlowUniformityIndex_Local[iMarker_Analyze] = 0.0;
    ...
    Surface_FlowUniformityIndex_Total[iMarker_Analyze] = 0.0;
    ...
    if (config->GetMarker_All_Analyze(iMarker) == YES)  {
      ...
      for (iMarker_Analyze= 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
        ...
        if (config->GetMarker_All_TagBound(iMarker) == config->GetMarker_Analyze_TagBound(iMarker_Analyze)) {
          ...
          Surface_FlowUniformityIndex_Local[iMarker_Analyze] += Surface_FlowUniformityIndex[iMarker];
        }
  ...
  SU2_MPI::Allreduce(Surface_FlowUniformityIndex_Local, Surface_FlowUniformityIndex_Total, nMarker_Analyze, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  ...
  for (iMarker_Analyze = 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
    ...
    Surface_FlowUniformityIndex_Total[iMarker_Analyze] = Surface_FlowUniformityIndex_Local[iMarker_Analyze];
  }
  ...
  for (iMarker_Analyze = 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
    ...
    su2double TotalFlowUniformityIndex = Surface_FlowUniformityIndex_Total[iMarker_Analyze];
    config->SetSurface_FlowUniformityIndex(iMarker_Analyze, TotalFlowUniformityIndex);
  ...
  if ((rank == MASTER_NODE) && !config->GetDiscrete_Adjoint() && output) {
    ...
    for (iMarker_Analyze = 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
      ...
      cout<<setw(18) <<"Flow_UI: "<< setw(10)<<config->GetSurface_FlowUniformityIndex(iMarker_Analyze);
      cout << endl;
  ...
  delete [] Surface_FlowUniformityIndex_Local;
  ...
  delete [] Surface_FlowUniformityIndex_Total;
  ...
  delete [] Surface_FlowUniformityIndex;
