...
def main():
    ...
    parser.add_option("-o", "--optimization", dest="optimization", default="SLSQP",
                      help="OPTIMIZATION techique (SLSQP, CG, BFGS, POWELL,IPOPT)", metavar="OPTIMIZATION")
    ...
    sys.stdout.write('|   / __| | | |_  )   Release 6.0.1 \"Falcon\"                            |\n')
...
def shape_optimization( filename                           ,
    ...
    if optimization == 'IPOPT':
      SU2.opt.IPOPT(project,config,state,x0,xb_low,xb_up,its,accu)
    if optimization == 'IPOPTMARKER':
      SU2.opt.IPOPTMARKER(project,config,state,x0,xb_low,xb_up,its,accu)
