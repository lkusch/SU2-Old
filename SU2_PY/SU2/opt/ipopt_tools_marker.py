...
def eval_f(x,user_data=None):
    project.config['MARKER_ANALYZE']='OUT'
    """ obj = eval_f(x)
...
def eval_g(x,user_data=None):
    project.config['MARKER_ANALYZE']='IN'
    """ cons = eval_g(x)
...
def eval_jac_g(x,flag, user_data=None):
	...
	assert len(x)==project.n_dv
	#infFile=open("/home/lkusch/inf",'a')
    	#infFile.write("eval_jac_g:"+str(x)+'\n')
    	#infFile.close()
...
def ipopt_run(projectx, config, state, x0=None,xb_low=None, xb_up=None, its=100,accun=1e-10):
