...
def eval_g(x,user_data=None):
    ...
    #calculate E(max(0,(f-fmean)/fmean+(g-gmean)/gmean))
    sumobj=0
    for count in range(0,nquad):
        ...
        if(liftdrag==1):
    	    ...
    	    cons[0]=cons[0]+1E-3*constraint
        objnew = max(0,(obj[0]-objmean)/objmean+(cons[0]-gmean)/(-gmean))
        #objnew = (obj[0]-objmean)/objmean+(cons[0]-gmean)/(-gmean)
        print "f: "+repr(obj[0])+" "+repr(objmean)+" "+repr(objnew)
        print "g: "+repr(cons[0])+" "+repr(gmean)
        sumobj=sumobj+weights[count]*objnew
    sumobj=sumobj/(sqrt(pi)) 
    #obj_scale = obj[obj.keys()[0]]['SCALE']
    print "sumobj: "+repr(sumobj)
   
    consnew=((sumobj)-0.15)*1E-3
	...
	sumdobj=0
    	for count in range(0,nquad):
            ...
            if(liftdrag==1):
    	        ...
    	        cons[0]=cons[0]+1E-3*constraint
            if(((obj[0]-objmean)/objmean+(cons[0]-gmean)/(-gmean))>0):
                dobjnew=(array(dobj[0],float_)*objmean-dobjmean*obj[0])/(objmean*objmean)+(-dcons[0]*gmean+dgmean*cons[0])/(gmean*gmean)
            else:
                dobjnew=0.0
