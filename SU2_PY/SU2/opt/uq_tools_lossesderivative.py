...
def eval_g(x,user_data=None):
    ...
    for count in range(0,nquad):
        ...
        #print obj[0]
        #print objmean
        objnew = max(0,(obj[0]-objmean)/objmean)
    ...
    #sumobj=sumobj/(objmean) #divide by objmean?
    ...
    for count in range(0,nquad):
        ...
        if(liftdrag==1):
    	    ...
    	    cons[0]=cons[0]+1E-3*constraint
        consnew = max(0,(cons[0]-gmean)/(-gmean)) #??
...
def eval_jac_g(x,flag, user_data=None):
	...
	sumdobj=0
    	for count in range(0,nquad):
            ...
            dobj = project.obj_df(x)
            obj=project.obj_f(x)
            if((obj[0]-objmean)/objmean>0):
                dobjnew=(array(dobj[0],float_)*objmean-dobjmean*obj[0])/(objmean*objmean)
            else:
                dobjnew=0.0
	...
	dgmean=dconsres[0] 
        ...
        for count in range(0,nquad):
            ...
            if(liftdrag==1):
    	        ...
    	        cons[0]=cons[0]+1E-3*constraint
            if((cons[0]-gmean)/(-gmean)>0):
                dconsnew = (-dcons[0]*gmean+dgmean*cons[0])/(gmean*gmean)
            else:
		dconsnew=0.0
