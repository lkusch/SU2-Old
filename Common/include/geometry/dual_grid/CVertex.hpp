...
protected:
  ...
  su2double Normal_Old[3] = {0.0};      /*!< \brief Storage for normal coordinates of the element and its center of gravity. */
...
public:
  ...
  /*!
   * \brief Get the normal (old) to a face of the control volume asociated with a vertex.
   * \return Dimensional normal vector, the modulus is the area of the face.
   */
  inline su2double *GetNormal_Old(void) override { return Normal_Old; }
  ...
  /*!
   * \brief Set the normal vector (stored in old).
   * \param[in] val_face_normal - Vector to initialize the normal vector.
   * \return Value of the normal vector.
   */
