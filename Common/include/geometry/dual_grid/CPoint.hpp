...
private:
  ...
  su2activematrix Coord_Store;            /*!< \brief storage vector with the coordinates of the node (for optimization). */
...
public:
  ...
  /*!
   * \brief Set the point coordinates for storage.
   * \param[in] iPoint - Index of the point.
   * \param[in] coord - Coordinate of the point.
   */
  inline void SetCoord_Store(unsigned long iPoint, const su2double *coord) {
    for (unsigned long iDim = 0; iDim < nDim; iDim++)
      Coord_Store(iPoint,iDim) = coord[iDim];
  }

  /*!
   * \brief Get the coordinates of the control volume (stored).
   * \param[in] iPoint - Index of the point.
   * \return pointer to the coordinate of the point.
   */
  inline su2double *GetCoord_Store(unsigned long iPoint) { return Coord_Store[iPoint]; }
};
